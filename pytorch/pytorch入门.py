...
# 4 张量的自动微分
# 说自了就是自动求号.
# 在要求导的变量定义中加上requires_grad=lrue表示要追踪这个变量的导数
...
# 反向传播
print(y.grad_fn)  # false

# 导数必须由标量输出创建，意思就是你要求导的最后结果，必须是一个标量，不能是向量
z = y.mean()  # 求平均
z.backward()  # 反向传，计算dz/dx,只有执行完backward之后，所有的requires_grad=True的变量的倒数都会被自动求出
print(x.grad)  # 0.25

# 如果不需要求导，可以把代码包在torch.no_grad中
print(x.requires_grad)  # True

# 通过with touch.no_grad() 暂时停止x求导
with torch.no_grad():
    print((x ** 2).requires_grad)


#detach(),返回与原始张量数据相同但脱离计算图的新张量,新张量requires_grad=False且与原始计算图无连接
print(y.requires_grad)#True

y = x.detach()
print(y.requires_grad)#Flase
